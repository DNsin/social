...
class UsersCreds:
    ...
    def user_registration(cls, data: UserReg) -> str:
        ...
        try:
            connector = psycopg2.connect(dbname='social', user='postgres', password='sinicin123')
            engine = connector.cursor()
            first_name, last_name, third_name, sex, datebirth, hobbies, city = user_dict["user_first_name"], user_dict['user_last_name'], user_dict['user_third_name'], user_dict['user_sex'], user_dict['user_birthday'], user_dict['user_hobby'], user_dict['user_city']
